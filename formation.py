...
class Formation:
    ...
    def createFormation(self, uav_points, formation_type='rastgele', uav_distance=0.25, center=Point(None, None, None), yaw=None):
        ...
        if self.N == 0:
            print("Warning: formasyon olustururken iha sayisi 0 olamaz",
                  file=sys.stderr)
            return
        ...
        try:
            ...
            print(
                f"{self.N} iha ile {formation_type} olusturma desteklenmiyor", file=sys.stderr)

    def __getcenter(self, center):
        x = np.mean([p.x for p in self.uav_points]
                    ) if center.x == None else center.x
        y = np.mean([p.y for p in self.uav_points]
                    ) if center.y == None else center.y
        z = np.mean([p.z for p in self.uav_points]
                    ) if center.z == None else center.z
        return Point(x, y, z)
    ...
    def kare(self, center, yaw, N, uav_distance):
        ...
        formation_points.append(
            Point(gx+uav_distance/2, gy+uav_distance/2, gz))
        formation_points.append(
            Point(gx+uav_distance/2, gy-uav_distance/2, gz))
        formation_points.append(
            Point(gx-uav_distance/2, gy+uav_distance/2, gz))
    ...
    def besgen(self, center, yaw, N, uav_distance):
        ...
        # noktalar
        formation_points.append(
            Point(gx,                      gy+0.8506508083520*Rate, gz))
        formation_points.append(
            Point(gx+0.8090169943749*Rate, gy+0.2628655560596*Rate, gz))
        formation_points.append(
            Point(gx-0.8090169943749*Rate, gy+0.2628655560596*Rate, gz))
        formation_points.append(
            Point(gx-0.5*Rate,             gy-0.6881909602356*Rate, gz))
        formation_points.append(
            Point(gx+0.5*Rate,             gy-0.6881909602356*Rate, gz))
    ...
    def yildiz(self, center, yaw, N, uav_distance):
        ...
        formation_points.append(Point(
            gx,                                  gy+0.8506508083520*Rate*bigStarRate, gz))
        formation_points.append(Point(
            gx-0.8090169943749*Rate*bigStarRate, gy+0.2628655560596*Rate*bigStarRate, gz))
        formation_points.append(Point(
            gx+0.8090169943749*Rate*bigStarRate, gy+0.2628655560596*Rate*bigStarRate, gz))
        formation_points.append(Point(
            gx-0.5*Rate*bigStarRate,             gy-0.6881909602356*Rate*bigStarRate, gz))
        formation_points.append(Point(
            gx+0.5*Rate*bigStarRate,             gy-0.6881909602356*Rate*bigStarRate, gz))

        # kucuk besgenin noktalari
        formation_points.append(Point(gx,
                                      gy-0.8506508083520*Rate*smallStarRate, gz))
        formation_points.append(Point(
            gx-0.8090169943749*Rate*smallStarRate, gy-0.2628655560596*Rate*smallStarRate, gz))
        formation_points.append(Point(
            gx+0.8090169943749*Rate*smallStarRate, gy-0.2628655560596*Rate*smallStarRate, gz))
        formation_points.append(Point(gx-0.5*Rate*smallStarRate,
                                      gy+0.6881909602356*Rate*smallStarRate, gz))
        formation_points.append(Point(gx+0.5*Rate*smallStarRate,
                                      gy+0.6881909602356*Rate*smallStarRate, gz))
    ...
    def hilal(self, center, yaw, N, uav_distance):
        ...
        formation_points.append(
            Point(gx-Rate*0.95105651629, gy-Rate*0.3090169943, gz))
        formation_points.append(
            Point(gx-Rate*0.40673664307, gy-Rate*0.9135454576, gz))
        formation_points.append(
            Point(gx+Rate*0.40673664307, gy-Rate*0.9135454576, gz))
        formation_points.append(
            Point(gx+Rate*0.95105651629, gy-Rate*0.3090169943, gz))
...
if __name__ == '__main__':
    ...
    formasyon.createFormation(
        ihalar[:4], 'kare', uav_distance=0.5, center=Point(None, None, 1))
    ...
    for i in range(formasyon.N):
        ax.scatter(formasyon.uav_points[i][0], formasyon.uav_points[i][1],
                   formasyon.uav_points[i][2], s=500, color='black', marker='$İHA$')
        ax.scatter(formasyon.formation_points[i][0], formasyon.formation_points[i]
                   [1], formasyon.formation_points[i][2], s=200, color='blue')
        ax.plot(
